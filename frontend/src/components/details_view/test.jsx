...
import { PERSON_API_ID } from '../../utils/constants';
import PersonEditForm from '../PersonEditForm'; // Import the edit form component

export default function DetailsContent({ contact, onContactUpdated }) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(`/persons/${contact.id}`, {
        method: 'PATCH',
        headers: {
      ...
      onContactUpdated(updatedContact); // Notify parent component with updated contact
      setIsEditing(false);
      setEditingField(null);
      setEditValue('');
      alert(`${editingField.replace(/_/g, ' ')} updated successfully!`);
    } catch (error) {
