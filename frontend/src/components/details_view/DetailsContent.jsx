...
export default function DetailsContent({ contact }) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      alert(
        `Failed to update ${editingField.replace(/_/g, " ")}: ${error.message}`
      );
    }
  };
  ...
  for (const key in contact) {
    if (Object.hasOwnProperty.call(contact, key)) {
      contactDeconstructed[key] =
        contact[key] === null || contact[key] === ""
          ? "not provided"
          : contact[key];
    }
  }
    ...
    <div className="bg-gray-100 rounded-lg shadow-md p-6 grid grid-cols-3 gap-4 h-full w-full">
      ...
      <div className="col-span-2 flex-col">
        ...
        <p>
          Last name:{" "}
          <span className="text-gray-500 text-sm mb-3">
            {contactDeconstructed.last_name}
          </span>
          <Tooltip title="Edit Company" arrow>
            <Button
              style={{ marginLeft: "6px" }}
